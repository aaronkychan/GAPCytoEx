...
import { refreshRelationsOutput, removeRelationsUsingArrows, renameArrowInRelations, selectRelation } from "./relation-ui";
...
function promptNameAndCheck(message: string, cyInstance: any, type: "vertex" | "arrow", state: WorkbenchState): string | null {
  const autoName = (document.getElementById("autoName") as HTMLInputElement | null)?.checked ?? false;
  if (autoName) {
...
function clickOnCanvas(event: any, cyInstance: any, state: WorkbenchState): void {
  ...
  if (state.mode === "delete") {
    if (target !== cyInstance) {
      ...
      if (target.isNode()) {
        removedArrows = target.connectedEdges().map((edge: any) => edge.id());
...
export function initCytoscape(state: WorkbenchState, inputData: { nodes?: CytoscapeElementData[]; edges?: CytoscapeElementData[] } | CytoscapeElementData[], isPreset = false): any {
  ...
  cyInstance.on("tap", (event: any) => clickOnCanvas(event, cyInstance, state));
  state.cy = cyInstance;
...
export function bendArrow(state: WorkbenchState, direction: "L" | "R" | "S"): void {
...
export function doubleQuiver(state: WorkbenchState): void {
  ...
  const existingArrowIds = new Set(state.cy.edges().map((edge: any) => edge.id()));
  const arrowsToDouble = state.cy.edges().filter((edge: any) => !edge.id().endsWith("*"));
...
export function createInitialVertexAtClick(state: WorkbenchState, event: MouseEvent): void {
  ...
  const element = { group: "nodes" as const, data: { id: name }, position: { x: event.offsetX - 40, y: event.offsetY - 40 } };
